...
export default function Repair() {
  ...
  const updateRepairData = async () => {
    ...
    if (
      newRepairData.status === repairData.status &&
      newRepairData.title === repairData.title
    ) {
      toast({
      ...
      return;
    } else if (changedStatus === "finished") {
    ...
    } else if (changedStatus === "registered") {
      response = await axios.put("/api/repair", {
        id: parseInt(repair_id),
        title: newRepairData.title,
        total_cost: repairData.total_cost,
